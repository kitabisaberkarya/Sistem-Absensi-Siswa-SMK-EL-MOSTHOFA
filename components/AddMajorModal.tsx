...
import { X, School, CheckCircle2, Hash } from 'lucide-react';
...
export const AddMajorModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await ApiService.createMajor(formData);
      setSuccess(true);
      setTimeout(() => {
        setSuccess(false);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-300 border border-gray-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h3 className="text-xl font-bold text-gray-900">Tambah Jurusan</h3>
                <p className="text-sm text-gray-500">Input program studi baru.</p>
             </div>
          </div>
        ...
        {success ? (
             <div className="p-12 flex flex-col items-center justify-center text-center animate-in zoom-in">
                 ...
                 <p className="text-gray-500 mt-2">Jurusan baru telah ditambahkan.</p>
             </div>
        ) : (
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Kode Jurusan</label>
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Nama Program Studi</label>
                    <input 
                        type="text" 
                        required
